...
- Glossaire et synonymes : repo, remote, local, git, github,
...
::: {.callout-important title="Pour résumer"}
- **remote** : dépôt distant = hébergé sur GitHub
  - il est central, tous les dépôts locaux sont branchés dessus
  - c'est la référence, la version officielle du projet
- **local** : dépôt sur votre propre machine = clone du remote
  - c'est ici que vous codez, testez, expérimentez

Une fois votre travail terminé en local, vous pouvez :

- envoyer vos modifications au remote
- supprimer votre travail s'il n'est pas concluant et revenir à une version antérieure
...
Vous allez travailler sur Git Bash inclus dans VSCode.
...
- [ ] Copiez le token (ghp_xxxxxxxxxx)
  - :warning: Attention sa valeur ne sera affichée qu'une seule fois. En cas de perte, il faut le supprimer et en recréer un nouveau.
...
::: {.callout-tip title="VSCode"}
Vous allez utiliser ce logiciel pour la majorité des cours d'informatique de l'ENSAI.

C'est un IDE (Integrated Development Environment) i.e. une application qui offre des outils complets pour le développement logiciel (Éditeur de Code, Compilateur / Interpréteur, Gestionnaire de Projet, Terminal Intégré...)
:::
...
- un prompt (username, host, folder)
  - vous êtes connecté avec l'utilisateur *onyxia* sur la machine vscode-python-345194-0
...
- [ ] Listez les fichiers et dossier du répertoire courant
...
- [ ] Créez les dossiers *src* et *doc*
  - dans VSCode > clic droit dans l'Explorer > New Folder
- [ ] Dans *src*, créez un fichier `voiture.py`
